...
class JobRepository extends CoreRepository
{
    ...
    public function getData($query){
        ...
        if(isset($query['email'])){
            if(isset($query['email']['where']['in'])){
                $query_where_in[] = [
                    'name' => 'user_id',
                    'array' => $query['email']['where']['in']
                ];
                $count_where_in = $count_where_in + 1;
            }else{
                $query_where[] = ['user_id', '=', $query['email']['where']];
            }
        }

        if(isset($query['cpabro_login'])){
            if(isset($query['cpabro_login']['where']['in'])){
                $query_where_in[] = [
                    'name' => 'user_id',
                    'array' => $query['cpabro_login']['where']['in']
                ];
                $count_where_in = $count_where_in + 1;
            }else{
                $query_where[] = ['user_id', '=', $query['cpabro_login']['where']];
            }
        }
